...
def test_invalid_two_poignees():
    ...
    with pytest.raises(ValueError):
        environment.step([Poignee(cards_list), Poignee(cards_list)])
...
def test_announce_simple_poignee_excuse_accepted():
    ...
    card_list = [Card.TRUMP_1, Card.TRUMP_2, Card.TRUMP_3, Card.TRUMP_4, Card.TRUMP_5, Card.TRUMP_6, Card.TRUMP_7,
                 ...
                 Card.TRUMP_16]
    observation = environment.step([Poignee(card_list)])[0]
    assert isinstance(observation["announcements"][3], Poignee)
...
def test_announce_simple_poignee_no_trump():
    ...
    environment.step([])
    with pytest.raises(ValueError):
        environment.step([Poignee(environment._hand_per_player[0][-10:])])
...
def test_announce_simple_poignee_no_such_cards_in_hand():
    ...
    with pytest.raises(ValueError):
        ...
        environment.step([Poignee(card_list)])
...
def test_announce_poignee_invalid():
    ...
    with pytest.raises(ValueError):
        environment.step([Poignee(get_card_list()[:11])])
    with pytest.raises(ValueError):
        environment.step([Poignee(get_card_list()[:14])])
    with pytest.raises(ValueError):
        environment.step([Poignee(get_card_list()[:16])])
